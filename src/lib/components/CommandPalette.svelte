...
{#if isOpen}
	...
	>
		...
		<div
			class="w-full max-w-xl overflow-hidden rounded-2xl border border-[#e3dedb] bg-[#fbf9f6]/95 backdrop-blur-xl shadow-2xl animate-in fade-in-0 zoom-in-95 duration-200"
			onclick={(e) => e.stopPropagation()}
			role="none"
		>
			<!-- Input Header -->
			<div class="relative flex items-center border-b border-[#e3dedb] px-4 py-3 bg-[#f7f3ee]">
				...
				<input
					...
					class="w-full bg-transparent text-sm font-mono text-[#241916] placeholder:text-[#7a665e] focus:outline-none"
				/>
				<kbd class="hidden sm:inline-block rounded border border-[#e3dedb] bg-white px-2 py-0.5 text-[10px] font-mono text-[#7a665e]">
					ESC
				</kbd>
			</div>
			...
			{#if copiedFeedback}
				<div class="bg-[#9a0002]/10 px-4 py-2 border-b border-[#9a0002]/30 flex items-center gap-2 text-xs font-mono text-[#9a0002]">
				...
				{:else}
					{#each filteredCommands as cmd}
						<button
							type="button"
							onclick={cmd.action}
							class="group flex w-full items-center justify-between rounded-xl px-3.5 py-2.5 text-left text-xs font-mono transition-all hover:bg-[#9a0002] hover:text-[#f7f3ee] cursor-pointer"
						>
							<div class="flex items-center gap-2.5">
								<span class="rounded bg-[#f7f3ee] px-2 py-0.5 text-[10px] text-[#7a665e] border border-[#e3dedb] group-hover:bg-[#7a0002] group-hover:text-[#f7f3ee] group-hover:border-[#7a0002]">
									{cmd.category}
								</span>
								<span class="text-[#241916] group-hover:text-[#f7f3ee] font-medium">
									{@html cmd.label}
								</span>
							</div>

							<div class="flex items-center gap-1 text-[#7a665e] group-hover:text-[#f7f3ee]">
			...
			<!-- Palette Footer -->
			<div class="flex items-center justify-between border-t border-[#e3dedb] bg-[#f7f3ee] px-4 py-2.5 text-[11px] font-mono text-[#7a665e]">
				<div class="flex items-center gap-3">
					<span>Use <kbd class="rounded bg-white px-1.5 py-0.5 border border-[#e3dedb]">⌘K</kbd> to toggle</span>
					<span>&bull;</span>
